...
## Files
- `build_report.py` — branded **HTML** report generator (stdlib only; no reportlab/ghostscript/qpdf). `python3 build_report.py <data.json> <report.html> [<cover.html>]` — writes the full report and, if a third path is given, a short cover note. Design is final — do not change layout/colors. Emits **table-based HTML with inline styles and `bgcolor` attributes** for every colored fill (KPI cards, red header bar, green/red pills, zebra, revenue bars), `border` for accents, a white-background wrapper, and the logo inlined as a small base64 data URI. This layout is deliberate: both render engines — Missive's email view and Google's `getAs('application/pdf')` converter — **ignore CSS `background-color` on divs/spans and mishandle `inline-block`**, but honor `bgcolor` on table cells and `border`. An earlier div + CSS-background version rendered with a deformed email body and a PDF that dropped the header bar and pills — that's why this is table-based.
...
- `Code.gs` — reference copy of the queue mailer (the live script is deployed in MegaRhino's Apps Script project; see `SETUP_GUIDE.md`). **Exactly two functions** differ from the shared Client Success mailer: `sendViaMissive_` now creates the Missive draft directly against `public.missiveapp.com/v1/drafts` (raw-base64 PDF, `send:false`) instead of POSTing to a webhook; and `buildAttachments_` gained an additive `PDF_FROM_HTML_GZ:<gz>::<name>.pdf` branch that decompresses a gzipped report HTML carried in the marker and renders the PDF from it (so the body can be a separate cover note). Every other function is byte-identical, and the existing `PDF_FROM_HTML` / URL / Drive-fileId attachment paths are unchanged. Editing this copy changes nothing until MegaRhino redeploys the Apps Script.
