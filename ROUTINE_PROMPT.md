...
**Config values:**
- `OUTBOX_FOLDER_ID = 1Va4VHJFydqAjq9piydQFdeVnPElszFDD` — the Google Drive folder the queue mailer polls. Drop each brand's job JSON here. (This is the SAME outbox every other project using the queue mailer writes to; the mailer processes every job regardless of type.)
- `LEDGER_FOLDER_ID = 1cR4lDuXpctVaA9q5Qs4VSrw9vsK95A9x` — the Drive folder holding the Weekly Sales dedup ledger `weekly_sales_sent.json`. **This is a DIFFERENT file from the mailer's other `sent_reports.json` ledger, and it must NOT live in the outbox** — the mailer treats every `.json` in the outbox as a job to send, so a ledger dropped there would be mis-sent and archived.
- Job send fields (written into every job): `sendApp = Missive` · `sendType = Draft` · `sendAs = support@megarhino.com` · `missiveLabels = ["7631fba9-76e7-4d7f-abf2-5f7d0d79d403"]` (the Weekly Sales Report shared label). The mailer leaves it as a Missive draft (never auto-sends) because `sendType` contains "draft".
...
job = {
    ...
    "missiveLabels": ["7631fba9-76e7-4d7f-abf2-5f7d0d79d403"],       # Weekly Sales Report shared label
}
...
- `missiveLabels` is the Missive **shared-label ID** applied to every Weekly Sales Report draft. The mailer passes it to Missive as `add_shared_labels`. It travels in the JOB JSON — never in the mailer's `CONFIG.MISSIVE_DEFAULT_LABEL_IDS` — because the outbox and queue are shared by many projects, and a mailer-level default would stamp this label on all of their drafts too. Keep it as a list with exactly this one ID unless told otherwise.

Then write it into the outbox with the Google Drive connector `create_file`:
- `parentId` = `OUTBOX_FOLDER_ID`
- `title` = `job_weekly_<CODE>_<YYYY-MM-DD>.json` (the `weekly_` prefix distinguishes it from other projects' jobs in the shared outbox; unique per brand + week)
